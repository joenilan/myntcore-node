...
/**
 * Provides a friendly event driven API to myntd in Node.js. Manages starting and
 * stopping myntd as a child process for application support, as well as connecting
 * to multiple myntd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Mynt(options) {
  if (!(this instanceof Mynt)) {
    return new Mynt(options);
  }
...
util.inherits(Mynt, Service);

Mynt.dependencies = [];

Mynt.DEFAULT_MAX_TXIDS = 1000;
Mynt.DEFAULT_MAX_HISTORY = 50;
Mynt.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Mynt.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Mynt.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Mynt.DEFAULT_SPAWN_RESTART_TIME = 5000;
Mynt.DEFAULT_SPAWN_STOP_TIME = 10000;
Mynt.DEFAULT_TRY_ALL_INTERVAL = 1000;
Mynt.DEFAULT_REINDEX_INTERVAL = 10000;
Mynt.DEFAULT_START_RETRY_INTERVAL = 5000;
Mynt.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Mynt.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Mynt.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Mynt.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Mynt.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Mynt.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Mynt.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Mynt.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Mynt.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Mynt.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Mynt.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Mynt.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Mynt.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Mynt.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Mynt.prototype._initCaches = function() {
...
Mynt.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Mynt.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Mynt.prototype.getPublishEvents = function() {
  return [
    {
...
Mynt.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'myntd/' + name, 'total:', this.subscriptions[name].length);
};

Mynt.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'myntd/' + name, 'total:', this.subscriptions[name].length);
};

Mynt.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    for(var i = 0; i < addresses.length; i++) {
        if (myntcore.Address.isValid(addresses[i], this.node.network)) {
            addAddress(addresses[i]);
        }
    }

    log.info(emitter.remoteAddress, 'subscribe:', 'myntd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Mynt.prototype.unsubscribeBalance = function(emitter, addresses) {
    var self = this;
    if(!addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'myntd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Mynt.prototype.unsubscribeBalanceAll = function(emitter) {
...
Mynt.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'myntd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Mynt.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'myntd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Mynt.prototype.unsubscribeAddressAll = function(emitter) {
...
Mynt.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Mynt.DEFAULT_CONFIG_SETTINGS;
...
Mynt.prototype._parseMyntConf = function(configPath) {
  var options = {};
...
Mynt.prototype._expandRelativeDatadir = function() {
...
Mynt.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in myntd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in myntd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in myntd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './mynt.conf');

  log.info('Using mynt config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseMyntConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseMyntConf(networkConfigPath));
  }
...
Mynt.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of myntcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of myntcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of myntcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to myntd from myntcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from myntd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from myntd. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in mynt.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that myntd is undergoing a reindex. ' +
             ...
             'of myntcore-node services will start.');
    node._reindex = true;
  }
};

Mynt.prototype._resetCaches = function() {
...
Mynt.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Mynt.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Mynt.prototype._initChain = function(callback) {
  var self = this;
...
Mynt.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8766
  };
  if (this.node.network === myntcore.Networks.testnet) {
    networkOptions.rpcport = 18766;
  }
  return networkOptions;
};

Mynt.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === myntcore.Networks.testnet) {
    networkPath = 'testnet3/mynt.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/mynt.conf';
    }
  }
  return networkPath;
};

Mynt.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === myntcore.Networks.testnet) {
...
Mynt.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('myntd/hashblock', message.toString('hex'));
    }
  }

};

Mynt.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Mynt.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Mynt Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Mynt.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Mynt.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Mynt.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('myntd/addressbalance', {
                    address: address,
                    txid: txid,
...
Mynt.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('myntd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Mynt.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = myntcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('myntd/rawtransaction', message.toString('hex'));
    }
...
Mynt.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Mynt.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Mynt.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Mynt.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Mynt.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Mynt Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Mynt.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Mynt.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Mynt.prototype._stopSpawnedMynt = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/myntd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned mynt process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean mynt process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Mynt.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedMynt(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting mynt process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Mynt process unexpectedly exited with code:', code);
        log.warn('Restarting mynt child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Mynt process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn myntd.'));
      }
...
Mynt.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to myntd.'));
    }
 ...
 */
Mynt.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Mynt configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Mynt.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Mynt.prototype.syncPercentage = function(callback) {
  var self = this;
...
Mynt.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, myntcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Mynt.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Mynt.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = myntcore.Script.fromAddress(delta.address);
    return {
...
Mynt.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Mynt.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Mynt.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in myntcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Mynt.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Mynt.prototype._getConfirmationsDetail = function(transaction) {
...
Mynt.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Mynt.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Mynt.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A mynt transaction id
 * @param {Function} callback
 */
Mynt.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Mynt.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof myntcore.Address) {
...
Mynt.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Mynt.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
...
+ * @param {Function} callback
+ */
Mynt.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Mynt.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Mynt.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Mynt.prototype.getRawBlock = function(blockArg, callback) {
...
Mynt.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Mynt.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Myntcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Mynt.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = myntcore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Mynt.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Mynt.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Mynt.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
Mynt.prototype.estimateSmartFee = function(blocks, conservative, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Mynt.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Mynt.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
+ * @param {Function} callback
+ */
Mynt.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Myntcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Mynt.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Mynt.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Mynt.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Mynt.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Mynt.prototype.getInfo = function(callback) {
  var self = this;
...
Mynt.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Mynt.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('myntd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('myntd process did not exit'));
      }
...
Mynt.prototype.getBlockReward = function(height) {
 ...
 * @param {Function} callback
 */
Mynt.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @return {*}
 */
Mynt.prototype.getMiningInfo = function(callback) {
    var self = this;
...
Mynt.prototype.getNetworkHash = function(height, callback) {
    var self = this;
...
Mynt.prototype.getPeerInfo = function(callback) {
    var self = this;
