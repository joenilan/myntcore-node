...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for myntd is necessary for this upgrade with the "reindex=1" mynt.conf option. \n' +
                 'There are changes necessary in both mynt.conf and myntcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/joenilan/myntcore-node/blob/master/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed myntcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './myntcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in myntcore-node services directory
    ...
    if (servicePackage.myntcoreNode) {
      serviceModule = service.name + '/' + servicePackage.myntcoreNode;
    }
 ...
 * @param {Node} node - The Myntcore Node instance
 */
