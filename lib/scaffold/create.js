...
var BASE_PACKAGE = {
  description: 'A full Mynt node build with Myntcore',
  repository: 'https://github.com/joenilan/myntcore',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
...
/**
 * Will create a base Myntcore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The mynt database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/myntcore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Myntcore Node directory, configuration file,
 * mynt configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the myntcore node configuration directory
 * @param {String} options.datadir - The path to the mynt datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the myntcore-node directory and configuration
    ...
    function(next) {
      // Setup the mynt directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createMyntDirectory(absDataDir, next);
      } else {
        next();
      }
    },
